...
class existAccessCardContent extends StatefulWidget {
  existAccessCardContent({super.key});

  @override
  existAccessCardContentState createState() => existAccessCardContentState();
}

class existAccessCardContentState extends State<existAccessCardContent> {
  ...
  OptionItem optionItemSelected = OptionItem(title: "Select Card");
  ...
  Future<void> getAccessCards() async {
    try {
      List<CardResponse> retrievedCards =
          await cardRepository.getAvailableAccessCard();
      setState(() {
  ...
  Future<void> loadExistData() async {
    try {
      setState(() {
